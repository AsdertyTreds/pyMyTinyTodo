...
function submitFullTask(form)
{
	...
	_mtt.db.request('fullNewTask', { list:curList.id, tag:_mtt.filter.getTags(), title: form.task.value, note:form.note.value,
			prio:form.prio.value, tags:form.tags.value, duedate:form.duedate.value, tag:_mtt.filter.getTags() }, function(json){
		if(!parseInt(json.total)) return;
		form.task.value = '';
		var item = json.list[0];
		taskList[item.id] = item;
...
{
	$.post(mytinytodo.mttUrl+'ajax?login', { login:1, password: form.password.value }, function(json){
		form.password.value = '';
		if(json.logged)
		{
			flag.isLogged = true;
...
function logout()
{
	$.post(mytinytodo.mttUrl+'ajax?logout', { logout:1 }, function(json){
		flag.isLogged = false;
		window.location.reload();
	}, 'json');
	return false;
...
function showSettings()
{
	...
	$('#page_ajax').load(_mtt.mttUrl+'settings?ajax=yes',null,function(){
